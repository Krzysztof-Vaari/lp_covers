class CoversController < ApplicationController
  ...
  # Uwierzyteknianie do czynności związanych z okładkami
  before_action :authenticate_user!, except: [:index, :show]
  before_action :correct_user, only: [:edit, :update, :destroy]
  # GET /covers
  # GET /covers.json
  def index
    @covers = Cover.all.order("created_at DESC").paginate(:page => params[:page], :per_page => 3)
  end
  ...
  private
    ...
    def cover_params
      params.require(:cover).permit(:description, :image, :name)
    end

    def correct_user
      @cover = current_user.covers.find_by(id: params[:id])
      redirect_to covers_path, notice: "Nie jesteś uprawniony do edycji tej okładki" if @cover.nil?
end
end
